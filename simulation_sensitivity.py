...
###specify detector masses for the scaling
###
mass=np.array([10.,25.,50.,100.,500.,1000.])
#in number of neutrino events, it will be rescaled into a detector mass
...
filename_dm='testrun_sensitivity.txt'
f=open(filename_dm,'w')
f.close()

###Chose further parameters in constants.py
...
if channel_Erec==True:
    if test==0:
        print ''
        print '2 d E_rec-cos_theta pdf for different times of the year'
        try:
            ...
            for ff in range (factor):
                H_tmp=recoil_dm(E_thr,m_DM_array[mm],t0,\
                                t1,N_t=N_tt,N_min=N_min_DM,mode=0)
                H_out=H_out+H_tmp
            print '         File created!'
...
for ard in range (len(mass)):
    ...
    if channel_Erec==True:
        ...
        if test==0:
            ...
            try:
                ...
                for ff in range (factor):
                    ...
                    for ii in range (3):
                        pdf_nu_tmp=recoil_nu(\
                            E_thr,ii,N_min=int(N_min_nu*ratio_arr[ii]),mode=0)
            ...
            for ii in range (len(x_edge)):
                p_margin_tmp=0.
                for jj in range (len(y_edge)-1):
                    p_margin_tmp+=0.5*(pdf_val[ii][jj]+\
                                        pdf_val[ii][jj+1])*dtheta
                p_margin.append(p_margin_tmp)
            norm=0.
            for ii in range (len(x_edge)-1):
                derec=x_edge[ii+1]-x_edge[ii]
        ...
        if test==1:
            ...
            Pb_noangle_ipl=interp1d(np.array([E_thr,upper_threshold]),\
                                        pdf_margin)
            pdf_nu_t=interp1d(time_array_nu,1./D_time*np.ones(\
                                    len(time_array_nu)),kind='linear')
    ...
    ### (min number of events for the pool of neutrinos is 10 for each species)
    print '         atmospheric...'
    t_src_atmo_nu=[]
    E_rec_src_atmo_nu=[]
    cos_src_atmo_nu=[]
    ...
    for mm in range (len(m_DM_array)):
        ...
        while scan==True:
            ...
            if jump==False:
                ...
                ###calculate expected dm events by multiplying with sigma
                ###
                if test==0:
                ...
                for ff in range (factor):
                    ...
                    if channel_time==True:
                        ...
                        for ii in range (9):
                            if N_sun_arr[ii]>0.:
                                ...
                                N_nu_solar=np.where(N_nu_solar<0.,0.,\
                                                    N_nu_solar)
                                mu_nu+=N_nu_solar
                        for ii in range (4):
                            if N_atmo_arr[ii]>0.:
                    ...
                    if channel_time==True:
                        ...
                        while ((len(t_array_DM))<NN_DM):
                            jj=np.random.randint(0,source_length-1,\
                                            source_length)
                    ...
                    for ii in range (9):
                        if N_sun_arr[ii]>0.:
                            N_nu_solar=np.random.normal(\
                                N_sun_arr[ii],nu_sigma[0]*N_sun_arr[ii],\
                                (N_sim,N_Q))
                            N_nu_solar=np.where(N_nu_solar<0.,0.,N_nu_solar)
                            mu_nu+=N_nu_solar
                    for ii in range (4):
                        if N_atmo_arr[ii]>0.:
                            N_nu_atmo=np.random.normal(\
                                N_atmo_arr[ii],nu_sigma[1]*N_atmo_arr[ii],\
                                (N_sim,N_Q))
                            N_nu_atmo=np.where(N_nu_atmo<0.,0.,N_nu_atmo)
                            mu_nu+=N_nu_atmo
                    for ii in range (3):
        ...
        #           poisson_SB=-(mu_DM+mu_nu)+(n_nu_arr+n_array_DM)*\
        #                   np.log(mu_DM+mu_nu)\
        #           -scipy.special.gammaln(n_nu_arr+n_array_DM+1)
                    ...
                    if channel_time==True:
                        ...
                        if channel_Erec==True:
                            ...
                            prob_DM_B_angle=Pb_ipl.ev(E_rec_array_DM,\
                                                        cos_array_DM)
                        ...
                        for ii in range (N_sim*N_Q):
                            time_info=1.+1.*mu_DM/mu_nu[ii]*\
                                    prob_DM_S_time[ii]/prob_DM_B_time[ii]
                            angle_info=1.+1.*mu_DM/mu_nu[ii]*\
                                    prob_DM_S_angle[ii]/prob_DM_B_angle[ii]
                            erec_info=1.+1.*mu_DM/mu_nu[ii]*\
                                    prob_DM_S_erec[ii]/prob_DM_B_erec[ii]
        
                            time_info=np.log(time_info)
                            angle_info=np.log(angle_info)
                            erec_info=np.log(erec_info)
        
                            pre=-mu_DM+(n_nu_arr[ii]+n_array_DM[ii])*\
                                    (np.log(mu_nu[ii])-np.log(mu_nu[ii]+mu_DM))
        
                            angle_info_DM.append(pre+np.sum(time_info)+\
                                            np.sum(angle_info))
                            erec_info_DM.append(pre+np.sum(time_info)+\
                                            np.sum(erec_info))
                    ...
                    if channel_time==True:
                        ...
                        for ii in range (N_sim*N_Q):
                            ...
                            Q_SB_angle=np.concatenate(\
                                    (Q_SB_angle,angle_info+angle_info_DM))
                            Q_SB_erec=np.concatenate(\
                                    (Q_SB_erec,erec_info+erec_info_DM))
                ...
                for kk in range (2):
                    ...
                    if print_info==1:
                        ...
                        #plt.figure(3)
                        #plt.hist(Q_SB,bins=np.linspace(\
                        #           min(Q_SB),max(Q_SB),100))
                    ...
                    ###
                    def pdf_B(Qv):
                        if Qv<np.min(Q_grid_B) or Qv>np.max(Q_grid_B)\
                                               or Qv==np.max(Q_grid_B):
                            return 0.
                        Qv=np.array([Qv,Qv])
