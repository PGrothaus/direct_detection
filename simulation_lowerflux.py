...
###
N_sim=50					#num of sets of events to be generated for simulation
N_Q=50					#num of times to vary the fluxes when evaluating Q
factor=50				#num of loops when generating the toy models
steps=2500				#num of steps for integrating the pdf's
N_bins=50
accuracy=0.0075				#amount by how much both Q_pdf integrals are required to be similar
N_tt=10					#num of lookup tables for DM, i.e. time bins
N_min_nu=15000				#num of created models to create 2d pdf
N_min_DM=10000				#num of created models to create 2d pdf
source_length=5000			#maximal number of toy models in pool to draw from per factor
...
###
#m_DM_array=np.array([8.,6.,4.,3.5])
#m_DM_array=np.array([5.,6.,7.,8.,9.,10.,11.,12.,15.,20.,30.,40.,50.,100.,250.,500.,1000.])
m_DM_array=np.array([1000.,500.,250.,100.,50.,30.,15.,11.,9.,8.,7.,6.,5.,4.,3.5])
sigma=np.logspace(-40,-52,120)#one sigma takes approx 1min with N_sim==25.000
filename_dm='FINAL_CF4_5keV_sigmadown_erec_1sigma.txt'
...
for mm in range (len(m_DM_array)):
	...
	while scan==True:
		...
		if jump==False:
			...
			for ff in range (factor):
				...
				if channel_time==True:
					...
					angle_info_DM,erec_info_DM=[],[]
	                                for ii in range (N_sim*N_Q):
	                                        time_info=np.array([np.sum(np.log(1.+1.*mu_DM/mu_nu[ii]*prob_DM_S_time[ii]/prob_DM_B_time[ii]))])
	                                        angle_info=np.array([np.sum(np.log(1.+1.*mu_DM/mu_nu[ii]*prob_DM_S_angle[ii]/prob_DM_B_angle[ii]))])
	                                        erec_info=np.array([np.sum(np.log(1.+1.*mu_DM/mu_nu[ii]*prob_DM_S_erec[ii]/prob_DM_B_erec[ii]))])
	
	                                        pre=np.array([-mu_DM+(n_nu_arr[ii]+n_array_DM[ii])*(np.log(mu_nu[ii])-np.log(mu_nu[ii]+mu_DM))])
						
	                                        angle_info_DM=np.concatenate((angle_info_DM,pre+time_info+angle_info))
	                                        erec_info_DM=np.concatenate((erec_info_DM,pre+time_info+erec_info))
				...
				if channel_time==True:
					...
					angle_info_nu,erec_info_nu=[],[]
	                                for ii in range (N_sim*N_Q):
	                                        time_info=np.array([np.sum(np.log(1.+1.*mu_DM/mu_nu[ii]*prob_nu_S_time[ii]/prob_nu_B_time[ii]))])
	                                        angle_info=np.array([np.sum(np.log(1.+1.*mu_DM/mu_nu[ii]*prob_nu_S_angle[ii]/prob_nu_B_angle[ii]))])
	                                        erec_info=np.array([np.sum(np.log(1.+1.*mu_DM/mu_nu[ii]*prob_nu_S_erec[ii]/prob_nu_B_erec[ii]))])
	
	                                        angle_info_nu=np.concatenate((angle_info_nu,time_info+angle_info))
	                                        erec_info_nu=np.concatenate((erec_info_nu,time_info+erec_info))
	
	                                Q_SB_angle=np.concatenate((Q_SB_angle,angle_info_DM+angle_info_nu))
	                                Q_SB_erec=np.concatenate((Q_SB_erec,erec_info_DM+erec_info_nu))
			...
			for kk in range (2):
				...
				###
				def pdf_B(Qv):
					if Qv<min(Q_grid_B) or Qv>max(Q_grid_B) or Qv==max(Q_grid_B):
						return 0.
					Qv=np.array([Qv,Qv])
		...
		if cl_erec<0.1 and mem1==0:#cl_erec<0.00001 and mem1==0:
			waszero=True
			mem1=1
	
		if cl_erec>0.1 and mem2==0:#cl_erec>0.1 and mem2==0:
			wasbig=True
			mem2=1
