...
###
norm_vector=[1.08*pp_norm,1.08*hep_norm,1.08*b8_norm,1.08*n13_norm,1.08*o15_norm,1.08*f17_norm,1.08*1000.*0.9*b7_norm,1.08*1000.*0.1*b7_norm,1.08*1000.*1.E7]
...
def get_flux_atmo(E_nu,typ):
	...
	E_nu=np.where(E_nu>Emax,Emax,E_nu)
	flux_out=t_var*flux_ipl(E_nu)
	return 1.1*flux_out#in (MeV cm^2 s)^-1
...
def get_flux_dsnb(E_nu,typ):
	...
	E_nu=np.where(E_nu>Emax,Emax,E_nu)
	flux_out=t_var*flux_ipl(E_nu)
	return 1.1*norm*flux_out#in (MeV cm^2 s)^-1
...
def recoil_nu(E_thr,typ,N_min=750000,array_length=1000,N_evt=1,mode=0,):
	if N_min==0: N_min=1
	###	(typ=0 solar, typ=1 atmo, typ=2 dsnb)
